...
(() => {
  ...
  const STORAGE_KEY = 'dermaMazeFungalProgressV1';
  const SESSION_SEED_KEY = 'dermaMazeFungalSeed';
  const questions = window.FUNGAL_QUESTIONS || [];
  const topics = window.FUNGAL_TOPICS || [];

  const copy = {
    ar: {
      'nav.home':'الرئيسية','nav.topics':'الموضوعات','nav.questions':'بنك الأسئلة','nav.results':'النتائج','nav.book':'الكتاب',
      'hero.title':'كل فطر له نمط.<br><strong>وكل نمط له قرار.</strong>',
      'hero.text':'صفحة مستقلة للفصل الفطري تجمع خريطة الموضوعات وبنكًا كاملًا من 150 سؤالًا ثنائي اللغة: معلومات، كيسات، علاج، وصور سريرية.',
      ...
      resetAllConfirm:'هل تريد مسح كل الإجابات والتقدم المحفوظ في الفصل الفطري والبدء من الصفر؟ ستظل الأسئلة المحفوظة في المفضلة كما هي.',
    ...
    en: {
      ...
      'hero.title':'Every fungus has a pattern.<br><strong>Every pattern guides a decision.</strong>',
      'hero.text':'A dedicated fungal chapter page with a topic map and a complete bank of 150 bilingual questions: knowledge, cases, treatment, and clinical images.',
      ...
      resetAllConfirm:'Clear every answered question and saved progress in the Fungal chapter and restart from zero? Bookmarks will stay saved.',
  ...
  function cacheElements(){
    [
      'funTopicJump','funTopicGrid','funModeSwitch','funTopicFilter','funTypeFilter','funDifficultyFilter','funSearch',
      'funApplyFilters','funRandomTen','funBookmarksOnly','funSessionTitle','funSessionCount','funScoreArc','funScorePercent',
      'funAnsweredCount','funCorrectCount','funSavedCount','funResetSession','funResetAllProgress','funProgressBar','funQuestionCard',
      'funQuestionId','funQuestionType','funQuestionDifficulty','funQuestionSource','funBookmarkBtn','funQuestionTopic',
      'funQuestionCounter','funQuestionImageWrap','funQuestionImage','funZoomImage','funQuestionText','funOptions',
      'funFeedback','funFeedbackIcon','funFeedbackTitle','funExplanation','funReasoning','funExamMessage',
      'funPrevQuestion','funSkipQuestion','funNextQuestion','funEmptyState','funExamFinish','funExamFinishTitle',
      'funFinishPercent','funFinishDetail','funFinishText','funReviewExam','funRestartExam','funOverallScore',
      'funCoverage','funStrongest','funReviewNext','funResultsGrid','funImageDialog','funCloseImage','funDialogImage',
  ...
  function setStaticTranslations(){
    document.querySelectorAll('[data-fun-i18n]').forEach(el => {
      const value = t(el.dataset.funI18n);
      if(value !== undefined) el.textContent = value;
    });
    document.querySelectorAll('[data-fun-i18n-html]').forEach(el => {
      const value = t(el.dataset.funI18nHtml);
      if(value !== undefined) el.innerHTML = value;
    });
    if(els.funSearch){
      els.funSearch.placeholder = lang()==='ar' ? 'Tinea، KOH، تيربينافين...' : 'Tinea, KOH, terbinafine...';
    }
  }
  ...
  function renderTopics(){
    ...
    els.funTopicJump.querySelectorAll('[data-jump]').forEach(btn => {
      btn.addEventListener('click', () => {
        const target = document.getElementById(`topic-${btn.dataset.jump}`);
        target?.scrollIntoView({behavior:'smooth',block:'center'});
      });
    });
    els.funTopicGrid.querySelectorAll('[data-start-topic]').forEach(btn => {
  ...
  function renderFilterControls(){
    ...
    els.funTopicFilter.value = currentTopic;

    const types = ['MCQ','Case','Treatment','Image'];
    els.funTypeFilter.innerHTML =
      `<option value="all">${escapeHtml(t('allTypes'))}</option>` +
      types.map(type => `<option value="${type}">${escapeHtml(typeLabel(type))} (${window.FUNGAL_STATS.types[type]})</option>`).join('');
    els.funTypeFilter.value = currentType;

    const diffs = ['Easy','Medium','Hard'];
    els.funDifficultyFilter.innerHTML =
      `<option value="all">${escapeHtml(t('allDifficulties'))}</option>` +
      diffs.map(d => `<option value="${d}">${escapeHtml(difficultyLabel(d))} (${window.FUNGAL_STATS.difficulties[d]})</option>`).join('');
    els.funDifficultyFilter.value = currentDifficulty;

    els.funSearch.value = state.filters.search;
    updateModeButtons();
    els.funBookmarksOnly.classList.toggle('active',state.bookmarksOnly);
    const heart = els.funBookmarksOnly.querySelector('span');
    if(heart) heart.textContent = state.bookmarksOnly ? '♥' : '♡';
  }

  function updateModeButtons(){
    els.funModeSwitch.querySelectorAll('[data-mode]').forEach(btn => {
      btn.classList.toggle('active',btn.dataset.mode === state.mode);
    });
  }

  function readControls(){
    state.filters.topic = els.funTopicFilter.value;
    state.filters.type = els.funTypeFilter.value;
    state.filters.difficulty = els.funDifficultyFilter.value;
    state.filters.search = els.funSearch.value.trim();
  }
  ...
  function showPractice(){
    ...
    els.funEmptyState.hidden = !empty;
    els.funQuestionCard.hidden = empty;
    els.funExamFinish.hidden = true;
    if(empty && state.bookmarksOnly){
      const p = els.funEmptyState.querySelector('p');
  ...
  function renderQuestion(){
    ...
    els.funQuestionId.textContent = q.id;
    els.funQuestionType.textContent = typeLabel(q.type).toUpperCase();
    els.funQuestionDifficulty.textContent = difficultyLabel(q.difficulty).toUpperCase();
    els.funQuestionSource.textContent = q.source;
    els.funQuestionTopic.textContent = lang()==='ar' ? q.topicAr : q.topicEn.toUpperCase();
    els.funQuestionCounter.textContent = `${String(state.index+1).padStart(2,'0')} / ${String(state.session.length).padStart(2,'0')}`;
    els.funQuestionText.textContent = local(q.question);
    els.funProgressBar.style.width = `${((state.index+1)/state.session.length)*100}%`;

    els.funBookmarkBtn.classList.toggle('saved',state.bookmarks.has(q.id));
    els.funBookmarkBtn.textContent = state.bookmarks.has(q.id) ? '♥' : '♡';

    if(q.image){
      els.funQuestionImageWrap.hidden = false;
      els.funQuestionImage.src = q.image;
      els.funQuestionImage.alt = `${q.id} ${local(q.question)}`;
    }else{
      els.funQuestionImageWrap.hidden = true;
      els.funQuestionImage.removeAttribute('src');
    }

    els.funOptions.innerHTML = displayOptions.map((option,index) => `
      <button class="fun-option" data-letter="${option.letter}">
        <span>${String.fromCharCode(65+index)}</span>
        <b>${escapeHtml(option[lang()] || option.en)}</b>
      </button>
    `).join('');

    els.funFeedback.className = 'fun-feedback';
    els.funExamMessage.hidden = true;

    els.funOptions.querySelectorAll('.fun-option').forEach(btn => {
    ...
    els.funPrevQuestion.disabled = state.index === 0;
    els.funNextQuestion.disabled = !answer && !state.reviewMode;
    els.funNextQuestion.querySelector('[data-fun-i18n="question.next"]')?.replaceChildren(
  ...
  function chooseAnswer(letter){
    ...
    els.funNextQuestion.disabled = false;
    updateSessionSummary();
    renderResults();
  }
  ...
  function markExamSelection(letter){
    ...
    els.funExamMessage.hidden = false;
  }
  ...
  function revealAnswer(q,selectedLetter){
    ...
    els.funFeedback.className = `fun-feedback show ${good ? 'good' : 'bad'}`;
    els.funFeedbackIcon.textContent = good ? '✓' : '×';
    els.funFeedbackTitle.textContent = good ? t('correct') : `${t('incorrect')} — ${t('correctAnswer')}: ${correctOptionText(q)}`;
    els.funExplanation.textContent = local(q.explanation);
    els.funReasoning.textContent = local(q.reasoning);
    els.funExamMessage.hidden = true;
  }
  ...
  function navigate(delta){
    ...
    state.index = next;
    renderQuestion();
    els.funQuestionCard.scrollIntoView({behavior:'smooth',block:'center'});
  }
  ...
  function finishSession(){
    ...
    els.funQuestionCard.hidden = true;
    els.funEmptyState.hidden = true;
    els.funExamFinish.hidden = false;
    els.funExamFinishTitle.textContent = state.mode === 'exam' ? t('examComplete') : t('studyComplete');
    els.funFinishPercent.textContent = `${result.percent}%`;
    els.funFinishDetail.textContent = `${result.correct} / ${result.answered} ${t('answeredWord')}`;
    els.funFinishText.textContent = state.mode === 'exam' ? t('examCompleteText') : t('studyCompleteText');
    updateSessionSummary();
    renderResults();
  }
  ...
  function reviewSession(){
    ...
    state.completed = false;
    els.funExamFinish.hidden = true;
    els.funQuestionCard.hidden = false;
    renderQuestion();
  }
  ...
  function updateSessionSummary(){
    ...
    els.funSessionTitle.textContent = sessionTitle();
    els.funSessionCount.textContent = `${state.session.length} ${t('questionsWord')}`;
    els.funAnsweredCount.textContent = s.answered;
    els.funCorrectCount.textContent = s.correct;
    els.funSavedCount.textContent = state.bookmarks.size;
    els.funScorePercent.textContent = `${s.percent}%`;
    els.funScoreArc.style.strokeDashoffset = String(314-(314*s.percent/100));
  }
  ...
  function renderResults(){
    ...
    els.funOverallScore.textContent = `${overall.percent}%`;
    els.funCoverage.textContent = `${overall.answered} / ${questions.length}`;
    els.funStrongest.textContent = strongest ? local(strongest.topic.title) : t('strongestEmpty');
    els.funReviewNext.textContent = review ? local(review.topic.title) : t('reviewEmpty');

    els.funResultsGrid.innerHTML = stats.map(item => `
      <article class="fun-result-card" style="--pct:${item.percent}%">
  ...
  function openImage(){
    const q = currentQuestion();
    if(!q?.image) return;
    els.funDialogImage.src = q.image;
    els.funImageDialog.showModal();
  }
  ...
  function bindEvents(){
    ...
    els.funApplyFilters.addEventListener('click',() => {
      state.bookmarksOnly = false;
      startSession({kind:'filtered'});
      renderFilterControls();
    });
    els.funRandomTen.addEventListener('click',startRandomTen);
    ...
    els.funBookmarksOnly.addEventListener('click',toggleBookmarksOnly);
    els.funPrevQuestion.addEventListener('click',() => navigate(-1));
    els.funNextQuestion.addEventListener('click',() => navigate(1));
    els.funSkipQuestion.addEventListener('click',skipQuestion);
    els.funBookmarkBtn.addEventListener('click',toggleBookmark);
    els.funResetSession.addEventListener('click',resetCurrentSession);
    els.funResetAllProgress?.addEventListener('click',resetAllProgress);
    els.funReviewExam.addEventListener('click',reviewSession);
    els.funRestartExam.addEventListener('click',restartSession);
    els.funZoomImage.addEventListener('click',openImage);
    els.funQuestionImage.addEventListener('click',openImage);
    els.funCloseImage.addEventListener('click',() => els.funImageDialog.close());
    els.funImageDialog.addEventListener('click',e => { if(e.target === els.funImageDialog) els.funImageDialog.close(); });
    els.funSearch.addEventListener('keydown',e => {
      if(e.key === 'Enter'){
        e.preventDefault();
        state.bookmarksOnly = false;
        startSession({kind:'filtered'});
